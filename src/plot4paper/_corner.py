...
class CornerPlot:
    ...
    def __histogram(
        ...
        nsf_err = np.maximum(
            self.__get_nSF(upper - median),
            self.__get_nSF(median - lower)
        )

        if quartiles[0]:
            ax.axvline(lower, color=color, ls=":", alpha=0.5)
        if quartiles[1]:
            ax.axvline(median, color=color, ls=":")
        if quartiles[2]:
            ax.axvline(upper, color=color, ls=":", alpha=0.5)

        if title and (ndp >= 0):
            vs = f"{{:{str(nsf_val + 1 + ndp)}.{str(ndp)}f}}"
            es = f"{{: >{str(nsf_err + 1 + ndp)}.{str(ndp)}f}}"
        ...
        if gaussian_prior is not None:
            grid = np.linspace(
                gaussian_prior[0] - 5.0 * gaussian_prior[1],
                gaussian_prior[0] + 5.0 * gaussian_prior[1],
                10000
            )
            gaus = np.exp(
                -0.5 * (grid - gaussian_prior[0])**2. / gaussian_prior[1]**2.
            ) / np.sqrt(2.0 * np.pi*gaussian_prior[1]**2.)
            # normalise
    ...
    def __2d(
        ...
        if use_sigma_regions_2d:
            """
            we sort the data and plot each pixel a different shade according
            to the sigma value....
            """
    ...
    def __update_histogram_y_limits(
            self,
            ax,
            newY
            ):
    ...
    def build_LL_axes(
        ...
        for i in range(ntheta):
            for j in range(i+1):
                ...
                # we can set the X-lim straight ahead as ALL subplots respect
                # the same rules
                self.__ax[k].set_xlim(axisRange[j])
                if ticks[j] is not None:
                    self.__ax[k].set_xticks(ticks[j])
                ...
                if i == j:
                    ...
                    if ticks[i] is not None:
                        self.__ax[k].set_yticks(ticks[i])
                ...
                if (i+1) == ntheta:
                    """
                        Bottom Row
                    """
                    self.__ax[k].xaxis.set_label_coords(
                        x=0.5,
                        y=-self.__lblpad
                    )
                ...
                if (j == 0) & (i > 0):
                    ...
                    self.__ax[k].yaxis.set_label_coords(
                        x=-self.__lblpad,
                        y=0.5
                    )
    ...
    def addData_ll(
        ...
        # ensure the data passed is the correct size for the axes constructed
        assert ntheta == self.__LL_ntheta
        ...
        if plotrange is None:
            plotrange = [
                self.__default_range(samples[:, i]) for i in range(ntheta)
            ]
        ...
        if nbin is None:
            # determine the number of bins we should use to
            # average 10 points per bin
            nbin = np.maximum(int(np.sqrt(nvalue / 10)), 10)

        # now we cycle over the samples
        for i in range(ntheta):
            for j in range(i + 1):
                k = f"LL_{str(i)}_{str(j)}"
                ...
                if i == (ntheta - 1):
                    ...
                    fxticks = [
                        self.__format2ndp(xtick, ndp[j]) for xtick in xticks
                    ]
                ...
                if (i > 0) & (j == 0):
                    ...
                    fyticks = [
                        self.__format2ndp(ytick, ndp[i]) for ytick in yticks
                    ]
        ...
        for i in range(ntheta):
            for j in range(i+1):
                ...
                if (j == 0) & (i > 0):
                    ...
                    # set the y axis label coordinates
                    self.__ax[k].yaxis.set_label_coords(
                        x=1.+self.__lblpad * 1.6,
                        y=0.5
                    )
                    # add y label to all rows except the first which requires
                    # only the first x-label
                    self.__ax[k].set_ylabel(label[i])
    ...
    def addData_ur(
        ...
        assert ntheta == self.__UR_ntheta
...
#     (
#         np.concatenate((
#             np.random.normal(loc=1.0, scale=0.3, size=500),
#             np.random.normal(loc=2.0, scale=0.1, size=100)
#         )),
#         np.concatenate((
#             np.random.normal(loc=1.5, scale=0.5, size=500),
#             np.random.normal(loc=3.0, scale=0.7, size=100)))
#     )
# )
...
#     label=label,
#     axisRange=[
#         (np.amin(x)-0.1*np.ptp(x), np.amax(x)+0.1*np.ptp(x))
#         for x in fake_data
#     ]
# )
